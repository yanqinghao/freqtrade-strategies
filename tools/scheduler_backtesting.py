...
class FreqtradeScheduler:
    def __init__(self, config_path, strategy_path, docker_compose_path, backtesting_strategy):
        ...
        self.bot_config_path = f"{docker_compose_path}/user_data/config.json"
    ...
    def download_data(self):
        ...
        command = (f"../.venv/bin/python -m freqtrade download-data --config {self.config_path} "
    ...
    def analyze_backtesting(self):
        """分析回测结果"""
        command = (f"../.venv/bin/python -m freqtrade backtesting-analysis -c {self.config_path} "
    ...
    def extract_summary_stats(self, backtesting_output):
        ...
        try:
            ...
            message += f"• 账户变化: {key_metrics['Starting balance']} → {key_metrics['Final balance']}\n\n"

            message += "📉 回撤统计:\n"
            message += f"• 账户回撤率: {key_metrics['Max % of account underwater']}\n"
            message += f"• 最大回撤额: {key_metrics['Absolute Drawdown']}\n"
            message += f"• 回撤区间: {drawdown_info['Drawdown high']} → {drawdown_info['Drawdown low']}\n"
            message += f"• 回撤时间: {drawdown_info['Drawdown Start']} → {drawdown_info['Drawdown End']}\n"
            ...
            for line in lines:
                if 'EXIT REASON STATS' in line:
                    found_exit = True
                    continue
                if found_exit and '│' in line and 'Exit Reason' not in line and 'TOTAL' not in line and '──────' not in line:
            ...
            for line in lines:
                if 'DAY BREAKDOWN' in line:
                    found_daily = True
                    continue
                if found_daily and '│' in line and '/' in line and 'Tot Profit' not in line and '──────' not in line:
                    parts = [p.strip() for p in line.split('│') if p.strip()]
                    if len(parts) >= 2:
                        date = parts[0]
                        profit = parts[1]
...
def main():
    ...
    backtesting_strategy = os.getenv('BACKTESTING_STRATEGY', 'LeverageSupertrend')
    docker_compose_path = os.getenv('DOCKER_COMPOSE_PATH', '/path/to/docker/compose')
